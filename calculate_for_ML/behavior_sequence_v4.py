...
# cate_id为1032或1010的单独占据32位
# 其他cate_id占据另外32位
if __name__ == '__main__':
  
  df = pd.read_csv('data/june/data_behavior_sequence_0608.csv', sep='\t')
  ...
  def separate_func(behavior_sequence):
     list_1032_1010 = []
     list_other = []
     for map_id in behavior_sequence:
       if len(list_1032_1010) < 32 and int(map_id) < 800:
         list_1032_1010.append(map_id)
       elif len(list_other) < 32 and int(map_id) >= 800:
         list_other.append(map_id)
       else:
         continue

     print(len(list_1032_1010))
     
     while len(list_1032_1010) < 32:
       list_1032_1010.append("0")
     while len(list_other) < 32:
       list_other.append("0")
     
     list_final = list_1032_1010 + list_other
     return list_final
  ...
  result_df["behavior_sequence"] = result_df["behavior_sequence"].apply(lambda row: separate_func(row)).apply(', '.join)
  print(result_df)
  
  # result_df.to_csv('data_behavior_sequence2_7d.csv', mode='a', index=False)
  # result_df.to_csv('data_behavior_sequence2_14d.csv', mode='a', index=False, header=False)
